...
void ATPG::transition_delay_fault_simulation(int &total_detect_num) {
  ...
  for (i = vectors.size() - 1; i >= 0; i--) {
    tdfault_sim_a_vector(vectors[i], current_detect_num,drop,detect);
    total_detect_num += current_detect_num;
    fprintf(stdout, "vector[%d] detects %d faults (%d)\n", i, current_detect_num, total_detect_num);
  }
}// fault_simulate_vectors

void ATPG::tdfault_sim_a_vector(const string &vec, int &num_of_current_detect,int &drop, bool &detect_any) {
  int i, nckt;
  fptr f;
  ...
  tdfault_sim_a_vector2(vec, num_of_current_detect,drop,detect_any);

}

void ATPG::tdfault_sim_a_vector_without_dropping(const string &vec, int &num_of_current_detect,int &drop, bool &detect_any) {
  int i, nckt;
  fptr f;
  detected_for_sim = false;
  for (i = 0; i < cktin.size(); i++) {
  ...
  tdfault_sim_a_vector2_without_dropping(vec, num_of_current_detect,drop,detect_any);

}

/* fault simulate a single test vector */
void ATPG::tdfault_sim_a_vector2(const string &vec, int &num_of_current_detect, int &drop, bool &detect_any) {
  wptr w, faulty_wire;
...
void ATPG::tdfault_sim_a_vector2_without_dropping(const string &vec, int &num_of_current_detect, int &drop, bool &detect_any) {
  wptr w, faulty_wire;
  /* array of 16 fptrs, which points to the 16 faults in a simulation packet  */
  fptr simulated_fault_list[num_of_pattern];
  fptr f;
